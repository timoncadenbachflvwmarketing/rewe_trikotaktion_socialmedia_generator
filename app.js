...
function buildResultCard(result) {
  ...
  if (navigator.share) {
    ...
    shareBtn.innerHTML = `${SHARE_ICON_SVG}<span>Teilen</span>`; // Icon + Text
    shareBtn.addEventListener("click", async () => {
      try {
        const blob = await canvasToBlob(canvas, "image/png");
        const file = new File([blob], `${format.filename}.png`, { type: "image/png" });
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: 'Dein FLVW Social Media Visual',
        ...
        if (err.name !== "AbortError") {
          ...
          showModal("Beim Teilen ist ein Fehler aufgetreten.", "Fehler");
        }
      }
    });
